import { Contract, getAccountByName } from 'secret-polar';
...
export default async function run() {
    ...
    const counter_deploy_response = await counter_contract.deploy(
        contract_owner
    );
    console.log(counter_deploy_response);

    const counter_contract_info = await counter_contract.instantiate(
        { count: 102 },
        'deploy test dugufgdu',
        contract_owner
    );
    console.log(counter_contract_info);

    const inc_response = await counter_contract.tx.increment({
    ...
    const factorial_deploy_response = await factorial_contract.deploy(
        contract_owner
    );
    console.log(factorial_deploy_response);

    const factorial_contract_info = await factorial_contract.instantiate(
        { factorial: 0 },
        'factorial test 2',
        contract_owner
    );
    ...
    const exec_response = await factorial_contract.tx.factorial(
        { account: contract_owner },
        4
    );
